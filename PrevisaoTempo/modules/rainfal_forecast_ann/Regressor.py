...
class Predict_Best():
    ...
    def set_predict_df_seaborn(self):
        for i in range(self.num_nets):
            ...
            results_predict.rename(columns = {0:'Anomaly'}, inplace = True)
    ...
    def start_predict_dfs_volume(self, filename):
        self.source_df_sum = self.read_data_set(filename)
        print('sum',self.source_df_sum)
        self.predict_df = self.predict_df*self.source_df_sum
        self.predict_df_seaborn = self.predict_df_seaborn*self.source_df_sum
    
    def save_predict_df_volume(self, filename):
        with open(filename, 'w') as file:
            print(self.predict_df_seaborn)
            self.predict_df_seaborn.to_csv(filename)
        with open(filename, 'w') as file:
...
if __name__ == '__main__':
    ...
    FILENAME_SOMA = '../../data/files/anninputs/somas/' + MONTH + '_' + TIME_GAP + 'soma.csv'
    FILENAME_PREDICT = '../../data/files/ann_output_seaborn/predict_comp_' + MONTH + '_' + TIME_GAP + '_regression_normalized.csv'
    FILENAME_PREDICT_SEABORN = '../../data/files/ann_output_seaborn/predict_comp_' + MONTH + '_' + TIME_GAP + '_regression_normalized_seaborn.csv'
    PB.save_predict_df_seaborn(FILENAME_PREDICT_SEABORN)
    PB.save_predict_df(FILENAME_PREDICT)
    
    FILENAME_SOMA_SEABORN = '../../data/files/ann_output_seaborn/' + MONTH + '_' + TIME_GAP + '_regression_volume.csv'
    
    
    PB.start_predict_dfs_volume(FILENAME_SOMA)
    PB.save_predict_df_volume(FILENAME_SOMA_SEABORN)
